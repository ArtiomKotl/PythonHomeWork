...
def get_data(files):
    ...
    for i in range(len(files)):
        ...
        with open(files[i], 'r') as f_obj:
            ...
            os_prod_list.append(os_prod_reg2.findall(data)[0].split()[2])
            os_prod_reg3 = re.compile(r'Код продукта:\s*\S*')
            os_prod_list.append(os_prod_reg3.findall(data)[0].split()[2])
            os_prod_reg4 = re.compile(r'Тип системы:\s*\S*')
            os_prod_list.append(os_prod_reg4.findall(data)[0].split()[2])
